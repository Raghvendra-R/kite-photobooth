...
const CourseDetail = () => {
  ...
  if (courseData === undefined) return <p className='text-center text-red-500 pt-20'>Course not found.</p>
  ...
  return (
    ...
    <div className='flex md:flex-row flex-col-reverse gap-10 
                    relative items-start justify-between 
                    md:px-36 px-8 md:pt-30 pt-20 text-left bg-gradient-to-b from-cyan-100/70'>
      ...
      <div>
        ...
        <p className='text-sm'>Course by <span className='text-blue-600 underline'>Donald Jackman</span></p>
            ...
            {courseData.courseContent.map((chapter, index) => (
              <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                ...
                <div className={`overflow-hidden transition-all duration-300 ${openSections[index] ? 'max-h-96' : 'max-h-0'}`}>
                  <ul className='list-disc md:pl-10 pl-4 pt-2 pb-2 text-gray-600 border-t border-gray-300'>
                    {chapter.chapterContent.map((lecture, i) =>
                      <li key={i} className='flex items-center gap-2 py-1 hover:bg-gray-100'>
                        <img src={assets.play_icon} alt="Play lecture" className='w-4 h-4 mt-1' />
      ...
      <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
             ...
             {
                playerData ? 
                <YouTube videoId={playerData.videoId} opts={{playerVars :{autoplay : 1}}} iframeClassName=' w-full aaspect-video'/>
        ...
        <div className='pt-5 px-4 pb-4'>
          <div className='flex items-center gap-2'>
                :<img className='w-3.5' src={assets.time_clock_icon} alt="time_clock_icon" iframeClassName=' w-full aaspect-video' />
            <p className='text-red-500'>5 days <span className='font-medium'>Left At This Price!</span></p>
          </div>

          <div className='flex gap-3 item-center pt-2'>
            <p className='text-gray-800 md:text-4xl text-2xl font-semibold'>{currency} {(courseData.coursePrice - (courseData.discount * courseData.coursePrice / 100)).toFixed(2)}</p>
            <p className='md:text-lg text-gray-500 linr-through'>{currency} {courseData.coursePrice}</p> 
            <p className='md:text-lg text-gray-500'>{courseData.discount}% off</p>
          </div>
            <div className=' flex item-center text-xm md:text-defult gap-4 pt-2 md:pt-4 text-gray-500'>
      ...
      <div>
        {/* Additional right side content if needed */}
      </div>
    </div>
                <Footer/>  
